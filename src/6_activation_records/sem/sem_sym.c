...
bool sem_sym_inuse(S_table t, S_symbol sym) { 
  ...
  while (k) {
    ...
    if (!strcmp(S_name(b->key), "<mark>") && 
        strlen(S_name(b->key)) == strlen("<mark>"))
      return false;

    if (!strcmp(S_name(b->key), S_name(sym)) && 
        strlen(S_name(b->key)) == strlen(S_name(sym)))
      return true;
